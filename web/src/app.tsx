...
function Metric({ label, children }: { label: string; children: React.ReactNode }) {
  return <div className="space-y-1 border-l-2 border-primary/30 pl-4"><span className="text-[10px] font-bold uppercase tracking-[.16em] text-muted-foreground">{label}</span><div className="font-mono text-lg font-semibold">{children}</div></div>
}
...
export default function App() {
  ...
  async function solve() {
    ...
    try {
      ...
      setCurrentInput(input); setReport(result)
  ...
  return (
    <div id="top" className="min-h-svh bg-background text-foreground">
          ...
          <div className="mx-auto max-w-7xl px-4 sm:px-6">
            ...
            <div className="grid gap-5 xl:grid-cols-[19rem_minmax(0,1fr)]">
              <Card className="h-fit"><CardHeader><CardTitle className="text-lg">Configuración</CardTitle><CardDescription>Origen, escenario y método principal.</CardDescription></CardHeader><CardContent className="space-y-5">
                ...
                <Separator />
                <div className="flex items-center justify-between gap-4"><div><Label htmlFor="production">Interpretación productiva</Label><p className="mt-1 text-xs text-muted-foreground">Exige X ≥ 0 para declarar el plan viable.</p></div><Switch id="production" checked={production} onCheckedChange={setProduction} /></div>
              </CardContent></Card>
              <Card><CardHeader><div className="flex flex-wrap items-center justify-between gap-3"><div><CardTitle>Matriz aumentada A · X = B</CardTitle><CardDescription>Acepta enteros, decimales, notación científica y fracciones.</CardDescription></div><Badge variant="outline">{A.length} × {A.length}</Badge></div></CardHeader><CardContent className="space-y-5">
                {source === "json" ? <div className="space-y-2"><Label htmlFor="json-input">Datos JSON</Label><Textarea id="json-input" className="min-h-60 font-mono text-xs" value={jsonInput} onChange={(event) => setJsonInput(event.target.value)} spellCheck={false} /><p className="text-xs text-muted-foreground">B puede ser vector o columna; escribe las fracciones como &quot;2/3&quot;.</p></div> : <div id="matrix-editor"><MatrixEditor A={A} B={B} onChange={(nextA, nextB) => { setA(nextA); setB(nextB); setReport(null) }} /></div>}
      ...
      {report && <TutorSheet open={tutorOpen} onOpenChange={setTutorOpen} input={currentInput} title={title} note={note} initialQuestion={tutorPrompt} stepContext={stepContext} />}
    </div>
  )
}
