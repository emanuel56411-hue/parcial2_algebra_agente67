    ...
    <Card className={report.status === "inconsistent" ? "border-rose-400/40" : negative ? "border-amber-400/40" : "border-primary/30"}>
      ...
      <CardContent className="space-y-6 pt-6">
        {report.solution ? <>
          ...
          <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-3">{report.solution.map((value, index) => <div key={index} className={`rounded-2xl border p-4 shadow-sm ${numeric(value) < 0 ? "border-rose-300/70 bg-rose-50/70 dark:border-rose-900/60 dark:bg-rose-950/20" : "border-violet-200/75 bg-gradient-to-br from-violet-50/80 to-sky-50/70 dark:border-violet-900/55 dark:from-violet-950/20 dark:to-sky-950/15"}`}><p className="mb-2 text-xs font-semibold uppercase tracking-wider text-primary"><MathFormula source={`x${index + 1}`} /></p><ExactValue value={value} showDecimal className="text-lg sm:text-xl" />{numeric(value) < 0 && report.production && <p className="mt-2 text-xs font-medium text-rose-700 dark:text-rose-200">No viable bajo X ≥ 0</p>}</div>)}</div>
