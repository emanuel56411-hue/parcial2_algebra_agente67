...
import { askTutor, solveExercise, solveSystem } from "@/lib/api"
...
export function TutorSheet({ open, onOpenChange, input, report, preferredMethod, stepContext }: Props) {
  ...
  async function submit() {
    ...
    if (!activeReport || looksLikeNewExercise) {
      ...
      try {
        const solved = await solveExercise(text)
        const nextMethod = solved.analysis.methods[selectedMethod] ? selectedMethod : "diagnosis"
        setAttached({ name: `Prompt interpretado · ${solved.variables.join(", ")}`, input: solved.input, report: solved.analysis })
        setMessages([
          { kind: "user", text },
          { kind: "attachment", name: `Ejercicio interpretado (${solved.variables.join(", ")})`, size: solved.analysis.A.length },
          { kind: "answer", answer: engineTutorAnswer(solved.analysis, nextMethod), source: "engine", method: nextMethod, action: "method" },
        ])
      } catch (caught) {
        setError(caught instanceof Error ? caught.message : "No pude convertir el ejercicio a una matriz.")
      } finally {
        setLoading(false)
      }
      return
