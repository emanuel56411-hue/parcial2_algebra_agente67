...
type ChatItem = { kind: "user"; text: string } | { kind: "welcome" } | { kind: "answer"; answer: TutorAnswer; source: TutorSource; method: string; action?: TutorAction }
...
export function TutorSheet({ open, onOpenChange, input, report, preferredMethod, stepContext }: Props) {
  ...
  async function submit() {
    ...
    if (!input) { setError("Revisa los datos de la matriz antes de continuar."); return }
    ...
    <SheetContent className="tutor-sheet flex flex-col gap-0 bg-[#10201c] p-0 text-foreground shadow-[0_30px_90px_#0009]" showCloseButton={false}>
      ...
      <ScrollArea className="min-h-0 flex-1 px-4 py-4" aria-label="Conversación con el tutor">
        <div className="space-y-4 pb-3" role="log" aria-live="polite" aria-relevant="additions">
          ...
          {messages.map((message, index) => message.kind === "user" ? <div key={index} className="ml-auto max-w-[85%] rounded-2xl rounded-tr-sm bg-primary px-4 py-3 text-sm text-primary-foreground whitespace-pre-wrap">{message.text}</div> : message.kind === "welcome" ? <div key={index} ref={index === messages.length - 1 ? latestAnswerRef : undefined} className="flex items-start gap-2.5"><TutorAvatar className="mt-0.5 size-8 border border-[#5fd4ff]" /><div className="max-w-[85%] space-y-2 rounded-2xl rounded-tl-sm border border-cyan-300/20 bg-cyan-300/10 px-4 py-3 text-sm leading-relaxed"><p>¡Hola! Me alegra ayudarte con matrices. Primero resuelve el sistema para que pueda explicarte sus pasos y valores exactos.</p><Button type="button" size="sm" variant="outline" onClick={() => { onOpenChange(false); window.setTimeout(() => document.getElementById("calculator")?.scrollIntoView({ behavior: reducedMotion ? "instant" : "smooth" }), 50) }}>Ir a la matriz</Button><p className="text-xs text-muted-foreground">Este mensaje no consume cuota.</p></div></div> : report ? <div key={index} ref={index === messages.length - 1 ? latestAnswerRef : undefined}><TutorAnswerView answer={message.answer} source={message.source} report={report} method={message.method} action={message.action} /></div> : null)}
      ...
      <form className="shrink-0 border-t border-cyan-300/20 bg-[#0d1715] px-4 pt-3 pb-[max(1rem,env(safe-area-inset-bottom))]" onSubmit={(event) => { event.preventDefault(); void submit() }}>
        ...
        {!report && <p className="mb-2 text-xs text-amber-200">Puedes escribirme ahora. Para hablar de cifras y pasos, primero resuelve la matriz.</p>}
        <Textarea ref={textareaRef} value={question} onChange={(event) => setQuestion(event.target.value)} maxLength={500} rows={3} placeholder={report ? "Escribe tu pregunta sobre el sistema actual…" : "Salúdame o pregunta cómo comenzar…"} aria-label="Pregunta para el Tutor IA" />
        <div className="mt-2 flex items-center justify-between gap-3"><small className="text-muted-foreground">{question.length}/500 · {report ? "Hasta cinco preguntas por minuto" : "Los saludos no consumen cuota"}</small><Button type="submit" disabled={loading || !question.trim()}>{loading ? <LoaderCircle className="animate-spin motion-reduce:animate-none" /> : <Send />}Enviar</Button></div>
      </form>
    </SheetContent>
  </Sheet>
}
